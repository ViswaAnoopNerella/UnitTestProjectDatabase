...
{
	[TestClass]
	public class UnitTestTraceAbility
	{
		readonly DataTracking.DAL.ITraceAbility TraceabilityOp;
		public DBLimitsExtractor dBLimitsExtractor;

		public UnitTestTraceAbility()
		{
			...
			dBLimitsExtractor = new DBLimitsExtractor();
		}
		...
		{
			string testlotbarcode1_actuatorassembly = "PT-67890+4148384+23510002A+25";
			string testlotbarcode2_actuatorassembly = "PT-12345+4148383+23510001A+20";
			string oldtestlotbarcode = "80600600+57805702+23330166B+200";
			var result = TraceabilityOp.GetLotInfoFromLotBarCode(testlotbarcode1_actuatorassembly);//23330166A
		...
		{
			var result = TraceabilityOp.GetOperatorInfoFromOperatorBarCode("2HCA1020444\r");
		...
		public void TestMethodFailAddMachineResult()
		{
			...
			var result = TraceabilityOp.AddMachineResult(lotInfo, "10.1+56+26+60+1+1+2017-01-25T13:15:30+0", "InsulationTest+RoomTempRef+RoomTempSensor+Measurements+Station+Verification+DateTime+Result");
		...
		public void TestMethodGetMachineSettings()
		{
			...
			var result = TraceabilityOp.GetMachineSettings(lotInfo,variableNames);

			//var result = TraceabilityOp.GetMachineSettings(lotInfo, "ProductType+ChipNest+CrimpTool+CylinderPressure+ChipDistance+ToleranceMinus+TolerancePlus");
			...
			if (result.Success)
			{
				...
				var MachineSettingDictionary = dBLimitsExtractor.ExtractSettings(variableNames,result.ResultObject);
				foreach (KeyValuePair<string, object> kvp in MachineSettingDictionary)
				{
		...
		[TestMethod]
		public void TestMethodGenerateSerialNumber()
		{

			var result = TraceabilityOp.GenerateSerialNumber();
			...
			if (result.Success)
			{
				Console.WriteLine("Serial Number:" + result.ResultObject);
			}
		...
		[TestMethod]
		public void TestMethodAddSerialNumber()
		{
			var lotInfo = new DataTracking.Models.LotInfo
			{
				LotIdent = 730251,
			...
			string serialNumber = "2330602246";

			var result = TraceabilityOp.AddSerialNumber(lotInfo, serialNumber);
		...
		[TestMethod]
		public void TestMethodCheckSerialNumber()
		{
			var lotInfo = new DataTracking.Models.LotInfo
			{
				LotIdent = 730251,
			...
			var result = TraceabilityOp.CheckSerialNumber(lotInfo, serialNumber);
		...
		[TestMethod]
		public void TestMethodGetProdLotComponentsWithBomQty()
		{
			var lotInfo = new DataTracking.Models.LotInfo();

			lotInfo.LotIdent = 61962790;

			var result = TraceabilityOp.GetProdLotComponentsWithBomQty(lotInfo);
		...
		[TestMethod]
		public void TestMethodAddMESMachineEvent()
		{
			var mesMachineEvent = new MESMachineEvent
			{
				MachineID = "W0022",
				StationID = 3,
				EventCode = "2",
				EventMessage = "Hardware Error Message 5",
				EventType = 1,
			...
			var result = TraceabilityOp.AddMESMachineEvent(mesMachineEvent);
			...
			if (result.Success)
			{
				Console.WriteLine("EventID:" + mesMachineEvent.EventID);
				Console.WriteLine("Success:" + result.Success);
			}
		...
		[TestMethod]
		public void TestMethodUpdateMESMachineEvent()
		{
			var mesMachineEvent = new MESMachineEvent
			{
				EventID = 3,
				OperatorID = "A1020444",
				IsResolved = true,
				DateTimeResolved = DateTime.Now,
				ResolutionNotes = "Resolved message and reason"
			};

			var result = TraceabilityOp.UpdateMESMachineEvent(mesMachineEvent);
